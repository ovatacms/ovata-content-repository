 ...
 * @author dani
 */
public class H2SearchProvider implements SearchProvider {

    private static final Logger logger = LoggerFactory.getLogger(H2SearchProvider.class);
    
    private final String databaseName;
    
    public H2SearchProvider( String databaseName) {
    ...
    public <T extends Query> T createQuery(Session session, Class<T> type) {
        if( QueryByExample.class == type) {
            return (T)new H2QueryByExample( this, session);
        }
        else if( FulltextQuery.class == type) {
            return (T)new H2FulltextQuery( this, session);
        }
        else {
